...
if ($submit){
    ...
    if( $result && $result->num_rows){
        ...
        if(password_verify( $password, $password_hash )){
            $sql = "Select image,image_thumbnail, text_size from users where user_name='$username'";
            $big_pic = $db->query($sql)->fetch_row()[0];
            $pic = $db->query($sql)->fetch_row()[1];
            $_SESSION['text_size'] = $db->query($sql)->fetch_row()[2];
            $_SESSION['username'] = $username;
            $_SESSION['profile_pic'] = $pic;
            $_SESSION["large_profile_pic"]= $big_pic;
            ob_clean();
            header("Location: /");
        }
    } else {
